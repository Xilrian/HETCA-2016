...
We use the true diversity index of order two to measure phenotypic and genotypic diversity at every iteration $t$ of the CA. Phenotypic diversity is therefore: $$^2D_p(t)=(\sum_{s=1}^5 (\frac{N(s,t)}{\sum_{s=1}^5 N(s,t)})^2)^{-1}$$ while genotypic diversity is: $$^2D_g(t)=(\sum_{g=1}^{K(t)} (\frac{N(g,t)}{\sum_{G=1}^{K(t)} N(G,t)})^2)^{-1}$$ with $K(t)$ number of distinct genotypes for iteration $t$ and $N(g,t)$ number of cells sharing the genotype $g$ at iteration $t$.
